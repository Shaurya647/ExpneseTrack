    ...
    inner class ExpenseViewHolder(private val binding: ItemExpenseBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(expense: Expense) {
            binding.apply {
                ...
                val tintRes = when (expense.category) {
                    "Food" -> R.color.food_icon
                    "Travel" -> R.color.travel_icon
                    "Shopping" -> R.color.shopping_icon
                    "Bills" -> R.color.bills_icon
                    "Entertainment" -> R.color.entertainment_icon
                    else -> R.color.other_icon
                }
                ivCategoryIcon.setColorFilter(androidx.core.content.ContextCompat.getColor(binding.root.context, tintRes))
