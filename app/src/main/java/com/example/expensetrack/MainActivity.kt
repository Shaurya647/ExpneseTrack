...
import com.example.expensetrack.databinding.DialogSetLimitBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnAnalysis.setOnClickListener { startActivity(Intent(this, AnalysisActivity::class.java)) }
    ...
    private fun showCustomLimitDialog(title: String, subtitle: String, currentLimit: Float, onSave: (Float) -> Unit) {
        val dialogBinding = DialogSetLimitBinding.inflate(layoutInflater)
        dialogBinding.tvDialogTitle.text = title
        dialogBinding.tvDialogSubtitle.text = subtitle
        if (currentLimit > 0) {
            dialogBinding.etLimitAmount.setText(currentLimit.toString())
        }

        dialogBinding.chip1000.setOnClickListener { dialogBinding.etLimitAmount.setText("1000") }
        dialogBinding.chip5000.setOnClickListener { dialogBinding.etLimitAmount.setText("5000") }
        dialogBinding.chip10000.setOnClickListener { dialogBinding.etLimitAmount.setText("10000") }
        dialogBinding.chip25000.setOnClickListener { dialogBinding.etLimitAmount.setText("25000") }
        ...
        dialogBinding.btnSaveDialog.setOnClickListener {
            val limit = dialogBinding.etLimitAmount.text.toString().toFloatOrNull() ?: 0f
            onSave(limit)
            dialog.dismiss()
        }
    ...
    private fun showCategoryLimitDialog(category: String) {
        ...
        showCustomLimitDialog(
            ...
            subtitle = "Define spending budget for $category",
    ...
    private fun showSetLimitDialog() {
        ...
        showCustomLimitDialog(
            ...
            subtitle = "Define total spending budget for this month",
            currentLimit = currentLimit
        ) { limit ->
    ...
    private fun loadDashboard() {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val monthFilter1 = SimpleDateFormat("MM/yyyy", Locale.getDefault()).format(Calendar.getInstance().time)
            val monthFilter2 = SimpleDateFormat("MMM yyyy", Locale.getDefault()).format(Calendar.getInstance().time)
            val monthlyExpenses = expenses.filter { it.date.contains(monthFilter1) || it.date.contains(monthFilter2) }
    ...
    private fun updateCategoryLimitUI(tvRemaining: android.widget.TextView, tvLimit: android.widget.TextView, total: Double, limit: Float) {
        if (limit > 0) {
            ...
            tvRemaining.setTextColor(
                ContextCompat.getColor(
                    this@MainActivity,
                    if (remaining >= 0) R.color.accent_teal else R.color.error
                )
            )
            ...
            tvRemaining.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.accent_teal))
